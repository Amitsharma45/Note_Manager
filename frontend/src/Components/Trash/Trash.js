...
function Trash(props) {
    ...
    useEffect(() => {
        const temp = data?.notes?.filter((i) => i.isTrash === true)
        settrashdata(temp)
    }, [data])
                ...
                <div className='lg:flex-[80%] md:flex-[70%] md:ml-12 flex-[100%]'>
                    <div className='  dark:bg-slate-700 bg-secondarylight rounded-xl  '>
                        <div className='flex justify-between dark:text-white text-2xl mx-5 pt-5'>
                            <span className='font-semibold text-3xl underline '>Trash Notes :-</span>
                                ...
                                <>
                                    {
                                        trashdata !== undefined && trashdata?.length !== 0 ? (
                                            <>
                                                {
                                                    trashdata !== undefined && trashdata?.map((item) =>
                                                        <TrashCard item={item} key={item._id} />
                                                )
                                                }
                                            </>
                                        ) : (
                                            <h1 className='text-center dark:text-white md:text-4xl text-2xl md:mt-11 mt-2'>There is not any Note in Trash!</h1>
                                        )
                                    }
                                </>
                            )
