...
function Card(props) {
    ...
    return (
        <div className='md:w-[350px] w-[300px] overflow-hidden h-min-[250px]  border-black bg-[#FBFBFB]  dark:bg-black dark:bg-opacity-10 dark:text-white text-slate-800  rounded-xl my-5 relative'>
            <div className='border-b-[1px] px-5 pt-4 pb-2 text-xl flex justify-between'>
                <div> {i.tittle}</div>
                <div className="dropdown inline-block relative">
                    ...
                    <ul className="dropdown-menu absolute hidden w-[100px] rounded-md bg-gray-300 py-1 dark:text-white dark:bg-slate-900 text-[16px]   left-[-60px]">
                        <li className="block  cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700 px-3" onClick={()=>settemp(!temp)} >
                            <FontAwesomeIcon icon={faPenToSquare} className='mr-2' />Edit
                        </li>
                        <li className="block  cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700 px-3 " onClick={() => DeleteNote(i._id)} >
            ...
            <textarea className='px-5 py-5 h-[180px] w-full bg-transparent resize-none outline-none' disabled={temp} value={i.notebody} />
